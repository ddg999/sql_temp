...
insert into movie (id, title, director, release_date, genre, rating)
values(1, '괴물', '봉준호', '2006-07-27', '드라마', 8.28),
...
-- 문제1. 장르가 드라마인 영화를 찾아주세요
select * from movie where genre = '드라마';

-- 문제2. id 4번의 영화 장르를 액션으로 수정해주세요
update movie set genre = '액션' where id = 4;

-- 문제3. 평점 9점 이상인 영화를 찾아주세요
select * from movie where rating >= 9;

-- 문제4. 밀양의 감독을 이창동, 개봉일을 2007년 5월 23일로 수정해주세요
update movie set release_date = '2007-05-23' where title = '밀양';

-- 문제5. 2017년에 개봉한 영화를 찾아주세요
select * from movie where release_date like '2017%';

-- 장르가 드라마 이고 id 가 6 이상인 영화의 평점을 8.67로 수정하기
update movie set rating = 8.67 where genre = '드라마' and id >= 6;
-- 관객수 컬럼을 추가하고 데이터를 넣어주세요
alter table movie add column people int;
